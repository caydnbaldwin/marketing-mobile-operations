...
└── lib/                            # functions (sourced)
    ...
    ├── launch_palera1n_loader.sh         # dvt launch in.palera.loader (best-effort foreground; DVT may background)
    ├── launch_sileo_app.sh               # uiopen sileo:// over dropbear (post-Sileo)
    ├── mark_local_network_granted.sh     # drops marker file via dropbear (post-operator-confirm)
    ...
    ├── verify_local_network_granted.sh   # probes for the Local Network marker via dropbear (silent)
    ...
    ├── verify_wifi_profile_installed.sh  # probes ProfileManifest.IsActive=true via pymobiledevice3 (silent)
...
- **Multi-flag chaining**: stage and standalone-verify flags can be passed together and run in order, aborting on the first failure (`set -e` semantics). `mmo -s1 -s1v -s2` is equivalent to the historical `mmo -s1 && mmo -s1v && mmo -s2`. Atomic ops that take positional args (`-sdl`, `-rsnps`) are still single-call — anything after the flag is treated as the op's arguments, not a subsequent flag — so they branch out of the multi-flag dispatch path entirely.
- **Stage `/v` shorthand**: `-s1/v`, `-s2/v`, `-s3/v` (and their `--stageN/v` long forms) expand to `-sN -sNv`, sugar for the common "run a main stage with its verify" chain. Composes with multi-flag dispatch: `mmo -s1/v -s2/v` ≡ `mmo -s1 -s1v -s2 -s2v`. Read the slash as "and" — `s1/v` is "stage 1 and its verify." Expansion happens in a preprocessor pass at the top of `run.sh`; the dispatch loop only ever sees canonical flags.
...
2. **WiFi install** *(manual bridge, skip-aware)* — pauses while operator taps Install in Settings > General > VPN & Device Management. Device auto-joins. Apple gates silent profile install behind device supervision (which requires wiping the phone), so we accept this tap on first runs.

   **Skip on re-runs**: gated on `verify_wifi_profile_installed`, which checks `ProfileManifest["com.stage2.wifi"].IsActive == true` from `pymobiledevice3 profile list`. `IsActive` flips true once the operator confirms in Settings — it's the lockdownd-side discriminator between "pushed but pending" and "actually installed and providing the WiFi config." Earlier the skip used `(verify_wifi_profile_pushed && get_wifi_ip)`, but `get_wifi_ip` needs OpenSSH on port 22, which isn't installed until step 4 — so the skip never fired on re-runs and the operator got prompted even when the phone was already on the WiFi. The `IsActive`-based check works pre-OpenSSH, no SSH required.
3. **Sileo install** *(semi-automated bridge)* — operator opens the palera1n app, taps Install Sileo, and sets the password to match `SSH_PASS` in `.env` (conventionally `alpine1`). On rootful palera1n with this build, the password sets on the `mobile` user, not `root` — SSH-as-root doesn't work on these phones regardless, so everything downstream targets `mobile@…`. **This step is also what unlocks dropbear authentication** — until mobile has a hash in `/etc/master.passwd`, nothing can SSH in (root is permanently locked with `!`). That's why bridges 4 and 5 must run *after* this one, not before. Just before showing the manual instructions, the script makes a best-effort `launch_palera1n_loader` (DVT) call — same channel `verify_palera1n_installed` uses, just without `--suspended`. After the operator presses Enter, the script auto-launches Sileo via `uiopen sileo://` over dropbear (`launch_sileo_app`) so iOS surfaces its WLAN-and-cellular data permission prompt — the operator needs to approve that, otherwise Sileo can't fetch sources and downstream apt operations break. Auto-launching is gated on `SILEO_INSTALL_RAN=1`: on a re-run where Sileo is already installed, the permission prompt has already been answered and re-opening is just visual noise.

   **Loader auto-launch caveat**: historically DVT launches apps in the *background* per Apple's Instruments protocol, so `launch_palera1n_loader` may spawn the process without visibly foregrounding the app. The "1. Open the palera1n app (if not already foregrounded)" instruction stays as a fallback. Worth re-evaluating against newer pymobiledevice3 / iOS combinations — the foregrounding behavior is Apple-side and can change. The loader has no `CFBundleURLTypes`, so a `uiopen palera1n://` hop isn't available pre-Sileo either.
...
5. **Local Network prompt** *(semi-automated, marker-gated)* — `trigger_local_network_prompt` runs `uiopen sms://` over the same dropbear tunnel, which brings Messages to the foreground and lets iOS surface the Local Network prompt. Operator still taps Allow on the prompt; the grant itself lives in `/private/var/preferences/com.apple.networkextension.plist` as an NSKeyedArchiver-serialized blob (NOT in TCC.db on iOS 15) and isn't safely shell-writable. Skipping this means the iMessageGateway tweak later fails with `EHOSTUNREACH` that looks like a network issue but isn't.

   **Marker-file idempotency**: because the actual grant is opaque, the step is gated on a sentinel file at `/var/mobile/Library/Preferences/.mmo_local_network_granted`. After the operator presses Enter, `mark_local_network_granted` drops the marker via dropbear; on subsequent runs `verify_local_network_granted` reads it and the whole step prints `[SKIP]` — no Messages launch, no prompt. The marker is a proxy for "the operator confirmed this once," not a guarantee iOS still has the grant. Failure mode: if the operator goes into Settings and revokes Local Network for Messages, the marker stays and stage 3 fails with `EHOSTUNREACH`. Recovery: `ssh mobile@<phone> rm /var/mobile/Library/Preferences/.mmo_local_network_granted` and re-run stage 2. Path choice rationale: `/var/mobile/Library/Preferences/` is mobile-owned (no sudo) and stable across reboots; `/tmp` would be wiped, root-owned paths would force sudo.
...
3. **Four atomic checks** (each is its own `lib/verify_*.sh` function):
   ...
   - `verify_sileo_installed` — `pymobiledevice3 apps query org.coolstar.SileoStore` + `jq -e` over USB/lockdownd. Used to use `apps list | grep`, but `apps list` returns ~1 MB of JSON that intermittently truncates a specific bundle's entry under USB load (right after `get_wifi_ip`'s iproxy cycle, in particular) — same byte count, missing Sileo. `apps query` returns just the requested bundle's metadata (a few KB) and is deterministic.
...
**Why `verify_palera1n_installed` uses `dvt launch`, not any of the metadata caches**: every lockdownd-side metadata source — `ideviceinstaller list`, `pymobiledevice3 apps list`, `pymobiledevice3 springboard icon`, `pymobiledevice3 developer dvt applist` — keeps stale entries after the bundle itself is gone. A phone that previously had palera1n and was later wiped, restored to stock, or chain-booted out without a clean uninstall will still report `in.palera.loader` as installed, with full metadata (`Path: /cores/binpack/Applications/palera1nLoader.app`, version, display name) and a valid icon PNG — even though the binpack volume isn't mounted, so the binary at that path is unreachable. The bug we hit was: stage 1's top-level guard saw the stale entry and printed `[SKIP]`, stage 1 verification's polling probe matched the same stale entry, and the script moved on to stage 2 with no actual palera1n on disk. The only probe that touches the binary is `pymobiledevice3 developer dvt launch --suspended in.palera.loader` — it attempts to exec the bundle, and a missing binary surfaces as exit 1 plus a clear `Failed to launch process with bundle identifier 'in.palera.loader'` error.

The probe runs `mounter auto-mount --version 15.5` (idempotent) then `dvt launch --suspended` *and* a follow-up `dvt process-id-for-bundle-id` cross-check — both are required for a "yes." The PID cross-check defends against several real failure modes that all surface as exit 0 from `dvt launch`: (1) DDI failed to mount (we hit this when iPhone reports 15.8 but Xcode has no 15.8 DDI on disk and pymobiledevice3 can't write the missing one to Xcode's protected `/Applications/Xcode.app/...` path); (2) DVT service failed to start for any other reason; (3) a transient state we observed once where DVT returned success without actually exec'ing the binary. In all three the launch exits 0 with no PID afterward — the cross-check turns "no PID" into return 1. Without it those silent failures propagated as false-positive `[SUCCESS] palera1n installed on device.` and let the script move past stage 1 verification with nothing on the phone (or with the phone actually jailbroken but the probe wrongly saying not). On a confirmed real launch we `dvt kill` the suspended PID before returning so we don't leave a stalled palera1n loader between probes (the polling loop in `wait_for_palera1n_installed` would otherwise leave one behind on the last poll iteration even with `--kill-existing` churning prior ones).

**Why `--version 15.5` is pinned**: `pymobiledevice3 mounter auto-mount` (no flag) wants to write the device-version-matched DDI into `/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/<version>` if that directory doesn't already have it. iPhone 6s on iOS 15.8 (our target) has no 15.8 DDI in Xcode by default, our user can't write to `/Applications/Xcode.app`, and pymobiledevice3 silently returns exit 0 on that permission failure. iOS DDIs are compatible across minor versions, so explicitly mounting an *older* one that's already on disk (15.5 ships with Xcode's stock DeviceSupport set) avoids the write entirely. If you re-target newer iOS, bump the version in both `verify_palera1n_installed.sh` and `launch_palera1n_loader.sh` to whatever's in `ls /Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/`.
