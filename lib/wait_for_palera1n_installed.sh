...
# Requires verify_palera1n_installed to already be sourced by the caller.
# Meant to be sourced. Do not execute directly.

wait_for_palera1n_installed() {
    echo "Waiting for palera1n to appear on device (up to 60s)..."
    ...
    echo "palera1n not found on device after 60s." >&2
    return 1
}
